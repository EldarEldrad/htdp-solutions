...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname Exercise_515) (read-case-sensitive #t) (teachpacks ((lib "abstraction.rkt" "teachpack" "2htdp") (lib "batch-io.rkt" "teachpack" "2htdp"))) (htdp-settings #(#t quasiquote repeating-decimal #f #t none #f ((lib "abstraction.rkt" "teachpack" "2htdp") (lib "batch-io.rkt" "teachpack" "2htdp")) #f)))
; A Lam is one of: 
; – a Symbol
...
(check-expect (undeclareds.v2 ex4) '((λ (x) (declared:x declared:x)) (λ (x) (declared:x declared:x))))
(check-expect (undeclareds.v2 '((λ (x) x) x)) '((λ (x) declared:x) *undeclared:x))
(check-expect (undeclareds.v2 '(λ (*undeclared) ((λ (x) (x *undeclared)) y)))
              '(λ (*undeclared) ((λ (x) (declared:x declared:*undeclared)) *undeclared:y)))
(define (undeclareds.v2 le0)
  (local (; Symbol -> Symbol
          (define (mark-declared s)
            (string->symbol (string-append "declared:" (symbol->string s))))
          ; Symbol -> Symbol
          (define (mark-undeclared s)
            (string->symbol (string-append "*undeclared:" (symbol->string s))))
