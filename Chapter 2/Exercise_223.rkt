...
#reader(lib "htdp-beginner-abbr-reader.ss" "lang")((modname Exercise_223) (read-case-sensitive #t) (teachpacks ((lib "image.rkt" "teachpack" "2htdp") (lib "universe.rkt" "teachpack" "2htdp"))) (htdp-settings #(#t write repeating-decimal #f #t none #f ((lib "image.rkt" "teachpack" "2htdp") (lib "universe.rkt" "teachpack" "2htdp")) #f)))
...
; moves dropping block left or right
(check-expect (tetris-key tetris0 "left") (make-tetris (make-block 4 3) landscape0))
(check-expect (tetris-key tetris0 "right") (make-tetris (make-block 6 3) landscape0))
(check-expect (tetris-key tetris0 "k") tetris0)
(check-expect (tetris-key tetris3 "left") (make-tetris (make-block 4 3) landscape3))
(check-expect (tetris-key tetris3 "right") (make-tetris (make-block 6 3) landscape3))
(check-expect (tetris-key tetris3 "k") tetris3)
(check-expect (tetris-key INITIAL "left") INITIAL)
(check-expect (tetris-key INITIAL "right") (make-tetris (make-block 1 0) landscape0))
(check-expect (tetris-key INITIAL "k") INITIAL)
(check-expect (tetris-key (make-tetris (make-block (- WIDTH 1) 6) '()) "left")
              (make-tetris (make-block (- WIDTH 2) 6) '()))
(check-expect (tetris-key (make-tetris (make-block (- WIDTH 1) 6) '()) "right")
              (make-tetris (make-block (- WIDTH 1) 6) '()))
(check-expect (tetris-key (make-tetris (make-block (- WIDTH 1) 6) '()) "k")
              (make-tetris (make-block (- WIDTH 1) 6) '()))
(check-expect (tetris-key (make-tetris (make-block 5 (- HEIGHT 1)) (list (make-block 4 (- HEIGHT 1)))) "left")
              (make-tetris (make-block 5 (- HEIGHT 1)) (list (make-block 4 (- HEIGHT 1)))))
(check-expect (tetris-key (make-tetris (make-block 5 (- HEIGHT 1)) (list (make-block 6 (- HEIGHT 1)))) "right")
              (make-tetris (make-block 5 (- HEIGHT 1)) (list (make-block 6 (- HEIGHT 1)))))
(define (tetris-key t ke)
  (cond
    [(and (key=? ke "right")
          (< (block-x (tetris-block t)) (- WIDTH 1))
          (not (member? (make-block (+ (block-x (tetris-block t)) 1) (block-y (tetris-block t)))
                        (tetris-landscape t))))
     (make-tetris (make-block (+ (block-x (tetris-block t)) 1) (block-y (tetris-block t)))
                  (tetris-landscape t))]
    [(and (key=? ke "left")
...
; Tetris -> Boolean
; returns #true if top of the canvas is reached
(check-expect (tetris-reached-top? tetris0) #false)
(check-expect (tetris-reached-top? tetris1) #false)
(check-expect (tetris-reached-top? tetris2) #false)
(check-expect (tetris-reached-top? tetris3) #false)
(check-expect (tetris-reached-top? INITIAL) #false)
(check-expect (tetris-reached-top?
               (make-tetris (make-block 5 0)
                            (list (make-block 0 0) (make-block 0 1) (make-block 0 2) (make-block 0 3)
                                  (make-block 0 4) (make-block 0 5) (make-block 0 6) (make-block 0 7)))) #true)
(define (tetris-reached-top? t)
  (landscape-reached-top? (tetris-landscape t)))

; Landscape -> Boolean
; returns #true if top of the canvas is reached
(check-expect (landscape-reached-top? landscape0) #false)
(check-expect (landscape-reached-top? landscape1) #false)
(check-expect (landscape-reached-top? landscape2) #false)
(check-expect (landscape-reached-top? landscape3) #false)
(check-expect (landscape-reached-top?
               (list (make-block 0 0) (make-block 0 1) (make-block 0 2) (make-block 0 3)
                     (make-block 0 4) (make-block 0 5) (make-block 0 6) (make-block 0 7))) #true)
(define (landscape-reached-top? l)
  (cond
    [(empty? l) #false]
    [(= 0 (block-y (first l))) #true]
    [else (landscape-reached-top? (rest l))]))
