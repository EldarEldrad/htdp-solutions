...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname Exercise_278) (read-case-sensitive #t) (teachpacks ((lib "image.rkt" "teachpack" "2htdp") (lib "batch-io.rkt" "teachpack" "2htdp") (lib "universe.rkt" "teachpack" "2htdp"))) (htdp-settings #(#t quasiquote repeating-decimal #f #t none #f ((lib "image.rkt" "teachpack" "2htdp") (lib "batch-io.rkt" "teachpack" "2htdp") (lib "universe.rkt" "teachpack" "2htdp")) #f)))
...
(define (render-positions lop image)
  (local ((define (draw-segment seg im)
            (place-image WORM-BODY
                         (* WORM-SIZE (- (* 2 (posn-x seg)) 1))
                         (* WORM-SIZE (- (* 2 (posn-y seg)) 1))
                         im)))
  (foldr draw-segment image lop)))
...
(define (hit-border?/list lop)
  (local ((define (hit-segment? seg)
            (or (<= (posn-x seg) 0)
                (> (posn-x seg) FIELD-SIZE)
                (<= (posn-y seg) 0)
                (> (posn-y seg) FIELD-SIZE))))
    (ormap hit-segment? lop)))
